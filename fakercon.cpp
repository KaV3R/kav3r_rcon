...
bool FakeRcon::Load(PluginId id, ISmmAPI *ismm, char *error, size_t maxlen, bool late)
{
	...
	if (CommandLine()->HasParm("-fakercon"))
	{
		...
		Debug("[KaV3R RCON] Fetching RCON from -fakercon command line");
	}
	else
	{
		g_szRconPassword = strdup(g_fileManager->GetRconPassword());
		Debug("[KaV3R RCON] Fetching RCON from %s", CONFIG_FILE);
	}

	if (!g_szRconPassword || strlen(g_szRconPassword) < 4)
	{
		Debug("[KaV3R RCON], please check the lengh of the password in game/csgo/%s or -fakercon command line", CONFIG_FILE);
	}
	else
	{
		Debug("[KaV3R RCON] Fake rcon is %s", g_szRconPassword);
	}
...
void Command_FakeRconCacheClean(const CCommandContext &context, const CCommand &args)
{
	...
	if (!pData->logged)
	{
		...
		{
			g_SMAPI->ClientConPrintf(slot, "First, enter the password with the kav3r_rcon_password command\n");
			return;
		}
...
CON_COMMAND_EXTERN(fake_rcon, Command_FakeRcon, "kav3r_rcon {CMD}");
...
CON_COMMAND_EXTERN(fake_rcon_password, Command_FakeRconPassword, "kav3r_rcon_password {CMD}");
void Command_FakeRconPassword(const CCommandContext &context, const CCommand &args)
{
	...
	if (pData->logged)
	{
		g_SMAPI->ClientConPrintf(slot, "You are already using the kav3r_rcon command\n");
		return;
	}
	...
	g_SMAPI->ClientConPrintf(slot, "You can now use the kav3r_rcon command\n");
	pData->logged = true;
...
{
	return "RCON cuz Valve is the best, thank you Valve XOXO";
}
...
{
	return "https://bio.catflux.hu/richardheimberger";
}
